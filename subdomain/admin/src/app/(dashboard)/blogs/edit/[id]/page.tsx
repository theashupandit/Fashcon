...
export default function EditBlogPage() {
      ...
      try {
        ...
        if (blogData) {
          setFormData({
            ...
            bottomBannerTitle: blogData.bottomBannerTitle || '',
            bottomBannerSubtitle: blogData.bottomBannerSubtitle || '',
            bottomBannerButtonText: blogData.bottomBannerButtonText || '',
            bottomBannerButtonUrl: blogData.bottomBannerButtonUrl || '',
          ...
          setBottomBannerImage(blogData.bottomBannerImage || null);
          ...
          // If no sections exist, add an empty one
  ...
  useEffect(() => {
    if (!loading && blogData && history.length === 0) {
      const initialState = { formData, sections, featuredImage, thumbnailImage, headerImage, bottomBannerImage, tags };
      setHistory([initialState]);
      setHistoryPointer(0);
    }
  }, [loading, blogData, formData, sections, featuredImage, thumbnailImage, headerImage, bottomBannerImage, tags, history.length]);
  ...
  }, [formData, sections, featuredImage, thumbnailImage, headerImage, bottomBannerImage, tags, loading, blogData]);
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(timer);
  }, [formData, sections, featuredImage, thumbnailImage, headerImage, bottomBannerImage, tags, loading, blogData, id, adProducts]);
  ...
  const addSection = () => {
    setSections([...sections, {
      title: `Step ${sections.length + 1}: New Step`,
      description: '',
      summary: '',
  ...
  if (loading) {
    return (
      <div className="h-[60vh] flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-[12px] font-black uppercase tracking-widest opacity-40">Loading Editorial Data...</p>
      </div>
    );
  }

  if (blogType === 'richtext') {
    return (
      <div className="max-w-[1400px] mx-auto space-y-6 pb-32">
        <div className="flex items-center gap-1 p-1 bg-[var(--muted)]/30 rounded-2xl border border-[var(--border)] w-fit mx-auto">
          <button onClick={() => setBlogType('infographic')} className={cn("flex items-center gap-2 px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all text-[var(--muted-foreground)] hover:bg-[var(--foreground)]/5")}>
            <Layout className="w-3.5 h-3.5" /> Infographic
          </button>
          <button onClick={() => setBlogType('richtext')} className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all bg-[var(--foreground)] text-[var(--background)] shadow-lg">
            <Type className="w-3.5 h-3.5" /> Rich Text
          </button>
        </div>
        <RichTextBlogEditor mode="edit" blogId={id as string} initialData={blogData} />
      </div>
    );
  }
  ...
  return (
    <div className="max-w-[1400px] mx-auto space-y-8 pb-32">
      ...
      <div className="flex items-center gap-1 p-1 bg-[var(--muted)]/30 rounded-2xl border border-[var(--border)] w-fit mx-auto">
        <button onClick={() => setBlogType('infographic')} className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all bg-[var(--foreground)] text-[var(--background)] shadow-lg">
          <Layout className="w-3.5 h-3.5" /> Infographic
        </button>
        <button onClick={() => setBlogType('richtext')} className={cn("flex items-center gap-2 px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all text-[var(--muted-foreground)] hover:bg-[var(--foreground)]/5")}>
          <Type className="w-3.5 h-3.5" /> Rich Text
        </button>
      </div>
        ...
        actions={
          <div className="flex flex-wrap items-center gap-3 justify-end">
            ...
            <Button 
              ...
              className="h-10 px-5 rounded-full border-red-500/20 text-red-500 hover:bg-red-500/5 font-black uppercase tracking-widest text-[9px] gap-2 transition-all active:scale-95 shrink-0"
            >
              <Trash2 size={14} />
            </Button>
            <Button 
              ...
              disabled={saving}
              className="h-10 px-5 rounded-full border-[var(--border)] text-[var(--foreground)] hover:bg-[var(--primary)]/5 font-black uppercase tracking-widest text-[10px] gap-2 transition-all active:scale-95 animate-in fade-in zoom-in-95 duration-150 shrink-0"
            >
              Save Draft
            </Button>
            <Button 
              ...
              disabled={saving}
              className="h-10 px-6 rounded-full bg-[var(--foreground)] text-[var(--background)] hover:opacity-90 font-black uppercase tracking-widest text-[10px] gap-2 shadow-lg transition-all active:scale-95 shrink-0"
            >
                    ...
                    >
                      {featuredImage ? (
                        <div className="flex items-center gap-2 text-[10px] font-bold">
                           <CheckCircle2 size={12} className="text-emerald-500" /> Selected
                        </div>
                    ...
                    <div className="flex flex-col md:flex-row">
                      ...
                      <div className="flex-1 p-8 space-y-6 relative">
                        ...
                        <div className="grid grid-cols-1 gap-4">
                          <div className="p-6 bg-[var(--background)] rounded-2xl border border-[var(--border)]/50 relative group/product min-h-[120px] flex flex-col justify-center">
                             ...
                             {section.productId ? (
                               <div className="flex items-center gap-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                                 <div className="w-16 h-16 rounded-xl border border-[var(--border)] overflow-hidden shrink-0 bg-[var(--muted)] relative shadow-sm">
                                   <SafeImage src={section.image} alt="Product" fill className="object-cover" />
                                 </div>
                                 <div className="flex-1 min-w-0">
                                   <p className="text-[12px] font-black truncate text-[var(--foreground)] tracking-tight">{section.ctaStore}</p>
                                   <p className="text-[10px] font-bold opacity-40 truncate uppercase tracking-widest">{section.ctaLabel}</p>
                                 </div>
                                 <div className="flex items-center gap-2">
                                   <Button 
                                     variant="ghost" 
                                   ...
                                   <Button 
                                     ...
                                     onClick={() => {
                                       ...
                                       setSections(newSections);
                                       toast.success("Product detached from step");
                                     }}
                                     className="h-9 w-9 rounded-xl bg-red-500/10 text-red-500 hover:bg-red-50 hover:text-white transition-all shadow-sm border border-red-500/20"
                                   >
                                     <Trash2 size={14} />
                                   </Button>
                                 </div>
                               ...
                               <div className="py-6 text-center border-2 border-dashed border-[var(--border)] rounded-xl opacity-20 group-hover/product:opacity-40 transition-all cursor-pointer" onClick={() => setShowProductPicker({ open: true, index: idx })}>
                                 <ShoppingBag size={20} className="mx-auto mb-2 opacity-20" />
                                 <p className="text-[10px] font-black uppercase tracking-widest italic">Click to Link Luxury Asset</p>
                               </div>
                             )}
                          </div>
                        ...
                        <div className="grid grid-cols-3 gap-4 pt-4 border-t border-[var(--border)]/50 items-center">
                          ...
                          <div className="space-y-2">
                            <Label className="text-[9px] font-black uppercase tracking-widest opacity-40">Store</Label>
                            <Input 
                          ...
                          <div className="space-y-2">
                            <Label className="text-[9px] font-black uppercase tracking-widest opacity-40">URL</Label>
                            <Input 
                ...
                return sections.map((section, idx) => {
                  ...
                  return (
                    <div key={idx} className={cn("flex flex-col md:flex-row gap-16 items-center text-[var(--foreground)]", idx % 2 === 1 && "md:flex-row-reverse")}>
                      <div className="w-full md:w-1/2 aspect-[4/5] rounded-[40px] overflow-hidden shadow-2xl bg-[var(--secondary)] relative">
                        <SafeImage 
                          src={section.image} 
                          alt={section.title} 
                          fill
