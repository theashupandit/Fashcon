    ...
    <>
      <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
        <DialogContent 
          ...
          className="!max-w-none !w-[calc(100%-240px)] !h-[calc(100%-80px)] !left-[calc(220px+((100%-220px)/2))] !top-[calc(62px+((100%-62px)/2))] !-translate-x-1/2 !-translate-y-1/2 p-0 overflow-hidden flex flex-col gap-0 bg-[#080808] border-white/5 rounded-[3rem] shadow-[0_0_100px_rgba(0,0,0,0.8)]"
        ...
        >
          {/* Main Top Header */}
          <header className="p-6 border-b border-white/5 shrink-0 bg-white/[0.02] flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <DialogTitle className="text-2xl font-black tracking-tighter uppercase italic text-white">Media Studio Pro</DialogTitle>
                <p className="text-[10px] font-bold opacity-30 uppercase tracking-[0.3em]">Centralized Asset Pipeline</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4">
              <div className="relative group min-w-[300px]">
                ...
                <input 
                  ...
                  className="w-full h-12 pl-12 pr-4 bg-white/[0.03] border border-transparent rounded-2xl font-bold text-[13px] text-white placeholder:text-white/20 focus:border-[var(--primary)]/30 focus:bg-white/[0.05] transition-all outline-none"
                />
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <label className="h-12 px-6 flex items-center gap-3 bg-white/5 border border-white/10 text-white text-[11px] font-black uppercase tracking-widest rounded-2xl cursor-pointer hover:bg-white/10 transition-all active:scale-95">
              ...
              <Button 
                ...
                size="icon" 
                onClick={onClose} 
                className="rounded-2xl h-12 w-12 bg-white/5 border border-white/5 text-white/40 hover:bg-red-500/20 hover:text-red-500 hover:border-red-500/30 transition-all"
              >
                <X className="w-5 h-5" />
              </Button>
            </div>
          ...
          <div className="flex-1 flex overflow-hidden">
            ...
            <aside className="w-72 border-r border-white/5 bg-white/[0.01] p-6 flex flex-col gap-6 overflow-y-auto scrollbar-hide">
              <div className="space-y-1">
                <h3 className="text-[10px] font-black uppercase tracking-widest opacity-30 ml-2 mb-3">Navigation</h3>
                  ...
                  {[
                    ...
                    <button 
                      ...
                      className={cn(
                        ...
                        nav.active ? "bg-[var(--primary)]/10 text-[var(--primary)]" : "text-white/40 hover:bg-white/5 hover:text-white"
                      )}
                    >
                      <nav.icon size={16} />
                      {nav.label}
              ...
              <div className="space-y-1">
                <h3 className="text-[10px] font-black uppercase tracking-widest opacity-30 ml-2 mb-3">Metadata Filters</h3>
                <div className="grid grid-cols-2 gap-2">
                  {['JPG', 'PNG', 'SVG', 'WEBP'].map((format) => (
                    <button key={format} className="h-9 border border-white/5 rounded-xl text-[10px] font-black text-white/30 hover:border-white/20 hover:text-white transition-all uppercase">{format}</button>
                  ))}
                </div>
              </div>

              <div className="mt-4 p-5 rounded-[1.5rem] bg-gradient-to-br from-[var(--primary)]/10 to-transparent border border-[var(--primary)]/10 text-center space-y-2 shrink-0">
                <div className="w-8 h-8 rounded-full bg-[var(--primary)]/20 flex items-center justify-center mx-auto text-[var(--primary)]">
                  <Sparkles size={14} />
                </div>
                <p className="text-[9px] font-bold text-white/60 leading-relaxed uppercase tracking-widest">Studio Tip</p>
                <p className="text-[8px] text-white/20 leading-relaxed">Drag and drop assets anywhere to begin the ingestion process.</p>
              </div>
            </aside>
            ...
            <main className="flex-1 overflow-y-auto relative p-8 scrollbar-hide">
              ...
              {loading ? (
                <div className="h-full flex flex-col items-center justify-center gap-4">
                  ...
                  <div className="text-center">
                    <p className="text-[11px] font-black uppercase tracking-[0.3em] text-white opacity-40">Scanning archives...</p>
                    <p className="text-[9px] font-bold text-white/10 uppercase tracking-widest mt-1">Retrieving Visual Metadata</p>
                  </div>
                </div>
                    ...
                    >
                      ...
                      <div className="absolute top-4 right-4 flex flex-col gap-2 translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all">
                        <Button 
                          size="icon" 
                          variant="secondary"
                          className="w-10 h-10 rounded-2xl bg-white/10 backdrop-blur-xl border border-white/10 text-white hover:bg-[var(--primary)] hover:border-[var(--primary)] transition-all"
                          onClick={(e) => {
                            e.stopPropagation();
                ...
                <div className="h-full flex flex-col items-center justify-center gap-6 py-20 opacity-20">
                  <div className="w-20 h-20 rounded-[2.5rem] border-2 border-dashed border-white/20 flex items-center justify-center">
                    <ImageIcon size={32} />
                  </div>
                  <div className="text-center">
                    <p className="text-sm font-black uppercase tracking-[0.3em]">No archives found</p>
                    <p className="text-[10px] font-medium mt-1">Try refining your scan parameters</p>
                  </div>
                </div>
              )}
              ...
              {isUploading && (
                <div className="absolute bottom-8 left-8 right-8 bg-white/5 backdrop-blur-3xl border border-white/10 rounded-[2rem] p-4 flex items-center gap-6 shadow-2xl animate-in slide-in-from-bottom-12 duration-500 z-[60]">
                  ...
                  <div className="flex-1 space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <p className="text-[11px] font-black uppercase tracking-widest text-white">Ingesting Assets</p>
                        <span className="text-[9px] font-bold text-white/30 uppercase tracking-widest bg-white/5 px-2 py-0.5 rounded-md">
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-[14px] font-black text-white">{uploadProgress}%</p>
                    <p className="text-[8px] font-bold text-white/20 uppercase tracking-[0.2em]">Synchronizing Archive</p>
                  </div>
                </div>
              )}
              ...
              {selectedAssetIds.length > 0 && !isUploading && (
                <div className="absolute bottom-8 left-1/2 -translate-x-1/2 bg-white/5 backdrop-blur-2xl border border-white/10 rounded-3xl p-4 flex items-center gap-6 shadow-2xl animate-in slide-in-from-bottom-8 duration-500 z-50">
                   <div className="flex items-center gap-3 px-2">
                     ...
                     <span className="text-[10px] font-black uppercase tracking-widest text-white/60">Assets Locked</span>
                   </div>
                   <div className="h-8 w-px bg-white/10" />
                   <div className="flex items-center gap-2">
                     <Button variant="ghost" onClick={() => setSelectedAssetIds([])} className="h-10 px-6 rounded-2xl text-[10px] font-black uppercase tracking-widest text-white/40 hover:text-white transition-all">Clear</Button>
                     <Button 
                       onClick={() => {
