...
export async function getSiteContent() {
  ...
  try {
    ...
    console.error('[site-content] MongoDB Connection failed, using fallback content:', error);
...
export async function getPinnedStoreProducts() {
  try {
    ...
    console.error('[site-content] Failed to fetch pinned store products, using empty rows:', error);
    return { row1: [], row2: [] };
  }
}
...
export async function getStorefrontSiteSettings() {
  try {
    ...
    console.error('[site-content] Failed to fetch storefront site settings, returning defaults:', error);
    return {
      seoTitle: 'Fashcon',
      seoDescription: 'Premium Curated Fashion Guides & Infographics',
